...
class ArticleViewSetTestCase(TestCase):
    ...
    def test_get_all_articles(self):
        ...
        self.assertEqual(
            json.dumps(response.data["results"]), json.dumps(serializer.data)
        )
    ...
    def test_get_all_articles_with_different_user(self):
        ...
        self.client.force_authenticate(user=self.user)
        response = self.client.get("/articles/")
        self.assertEqual(json.dumps(response.data["results"]), "[]")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_create_invalid_article(self):
        response = self.client.post("/articles/", data=self.invalid_article_payload)
    ...
    def test_get_articles_comments_by_user(self):
        ...
        self.assertEqual(
            response.data["results"][0]["title"], self.article1_comment1.title
        )
    ...
    def test_remove_comment_report(self):
        ...
        self.assertEqual(response.data["report_count"], 1)
        response = self.client.post(
            f"/comments/{self.article1_comment1.id}/remove_report/"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
